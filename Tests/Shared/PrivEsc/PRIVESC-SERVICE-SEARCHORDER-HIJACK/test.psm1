...
# Notes   : V5: injects -TestId and keeps Core enrichment valid
# ================================================================
function fncGetPrivEscServiceAndDllCorrelations {
    ...
    $testId = "PRIVESC-SERVICE-SEARCHORDER-HIJACK"

    # ------------------------------------------------------------
    # Exploitation Narrative
    # ------------------------------------------------------------
    $exploitationText = @"
Writable, high-privilege owned directories that participate in DLL or binary search order
can be abused to hijack execution flow. Attackers may drop a malicious DLL/EXE into a directory
that is searched before the legitimate location (service directory, PATH precedence), or target
ServiceDll runtime loads defined under service registry Parameters. Restarting a service or invoking
a process that uses the affected search order can result in code execution in the service context
(often SYSTEM), enabling privilege escalation.
"@
    ...
    # ------------------------------------------------------------
    $remediationText = @"
- Remove write access for low-privileged users/groups from service binary directories and ServiceDll directories.
- Ensure service binaries and their directories are owned by SYSTEM/Administrators/TrustedInstaller where appropriate.
- Review PATH environment variable ordering; remove writable directories and avoid placing user-writable locations before System32.
- Monitor service configuration and DLL loads; validate DLL integrity and enforce allowlisting where possible (WDAC/AppLocker).
- Validate service permissions (who can restart/change config) and apply change control for service modifications.
"@
    ...
    function fncTokenCanWritePath {
        ...
        try {
            ...
            foreach ($ace in $acl.Access) {
                ...
                if ($rights -match "FullControl|Modify|Write") { return $true }
                if ($rights -match "WriteData|CreateFiles|AppendData") { return $true }
            }
        } catch {}
        ...
        switch ($SurfaceType) {
            "ServiceBinary" {
                ...
                if ($StartName -match "LocalSystem") {
                    $impact = "Instant SYSTEM"; $effort = "Low"; $stealth = "Low"
                }
                elseif ($StartName -match "LocalService|NetworkService") {
                    $impact = "Service Account Privilege"; $effort = "Low"; $stealth = "Low"
                }
            }
            "ServiceDir" {
    ...
    function fncPrintSurface {
        ...
        foreach ($i in $Items) {
            ...
            Write-Host ("$prefix [{0}/100 | {1} | {2}] {3}" -f $i.Score,$i.Severity,$i.Confidence,$i.Summary) -ForegroundColor $colour

            if ($i.SurfaceType) {
                $rt = fncGetOffsecContext -SurfaceType $i.SurfaceType -StartName $i.StartName -BeforeSystem32 $i.BeforeSys
                Write-Host ("       Offsec: Impact={0} `n       Trigger={1} `n       Effort={2}  `n       Stealth={3}" -f $rt.Impact,$rt.Trigger,$rt.Effort,$rt.Stealth) -ForegroundColor DarkGray
            }
        }
    ...
    $pathEntries = @()
    try { $pathEntries = ($env:PATH -split ";") | Where-Object { $_ } | ForEach-Object { $_.Trim() } | Where-Object { $_ } } catch { $pathEntries = @() }
    ...
    for ($i = 0; $i -lt $pathResolvedByIndex.Count; $i++) {
        ...
        try { if (-not (Test-Path -LiteralPath $dir -PathType Container -ErrorAction SilentlyContinue)) { continue } } catch { continue }
    ...
    # ==========================================================
    # Services: correlate service binary + service directory
    # ==========================================================
    $svcDirSurfaces = @()
    $svcBinSurfaces = @()
    ...
    foreach ($svc in $services) {
        ...
        $svcIdx++
        if (($svcIdx % 25) -eq 0) {
            Write-Progress -Id 21 -Activity "Correlating services (binary + directory)" -Status ("{0}/{1} services" -f $svcIdx,$svcTotal) -PercentComplete ([int](($svcIdx / [Math]::Max($svcTotal,1)) * 100))
        }
    ...
    if (Test-Path $svcRegRoot -ErrorAction SilentlyContinue) {
        ...
        foreach ($k in $svcKeys) {
            ...
            $idx++
            if (($idx % 100) -eq 0) {
                Write-Progress -Id 22 -Activity "Correlating ServiceDll runtime loads" -Status ("{0}/{1} services" -f $idx,$total) -PercentComplete ([int](($idx / [Math]::Max($total,1)) * 100))
            }
