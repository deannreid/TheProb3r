...
function fncGetDefenseEvasionAVBypassAttackSurface {
    ...
    if (-not $surfaces -or $surfaces.Count -eq 0) {
        fncPrintMessage "No Defender exclusions found (or access denied to exclusion keys)." "success"

        fncAddFinding `
            -Id "DEF_EVASION_AVBYPASS_NONE" `
            -Category "Defense Evasion" `
            -Title "No Defender Exclusion Attack Surfaces Detected" `
            -Severity "Good" `
            -Status "Not Detected" `
            -Message "No Microsoft Defender exclusions were enumerated from standard exclusion registry locations." `
