...
Attackers commonly:
- Exploit publicly documented vulnerabilities
- Use automated exploit frameworks
- Target unpatched legacy components
- Chain with privilege escalation flaws
"@

$remediation = @"
Upgrade or remove this software.

Actions:
- Replace with supported version
- Remove legacy dependencies
- Block execution if removal not possible
- Validate no business-critical dependency exists
"@

    fncAddFinding `
        -Id ("EOL_" + ($e.Name -replace '[^A-Za-z0-9]','')) `
        -TestId "RECON-INSTALLED-SOFTWARE-INVENTORY-ADV"
...
$exploitation = @"
This vulnerability is listed in the CISA Known Exploited Vulnerabilities (KEV) catalogue.

Indicates:
- Active exploitation in the wild
- Weaponised exploit code likely exists
- High probability of compromise if exposed
"@
...
"@
    ...
    fncAddFinding `
        ...
        -Severity "Critical" `
        -Status "Detected" `
        -Message ("{0} affected by {1} (CVSS {2})." -f $e.Name,$e.TopCveId,$e.TopCvss) `
        -Exploitation $exploitation `
        -Remediation $remediation `
...
- Apply virtual patching (WAF/IPS)
...
$exploitation = @"
Offensive security tooling detected.
...
Actions:
- Confirm authorised red team usage
- Remove if unauthorised
- Monitor usage via EDR
- Restrict execution via AppLocker
"@
    ...
    fncAddFinding `
        ...
        -Exploitation $exploitation `
        -Remediation $remediation `
        -Recommendation "Validate or remove."
}
...
$exploitation = @"
Pivot-capable tooling enables remote access or tunnelling.
...
- Monitor usage logs
- Remove unauthorised installations
"@
    ...
    fncAddFinding `
        ...
        -Exploitation $exploitation `
        -Remediation $remediation `
        -Recommendation "Enforce MFA and restrict access."
}
...
$exploitation = @"
Unsigned binaries increase risk of:
...
"@
    ...
    fncAddFinding `
        ...
        -Exploitation $exploitation `
        -Remediation $remediation `
        -Recommendation "Validate binary integrity."
}

# ===============================
# Elevated Exploit Score
# ===============================
if ($e.ExploitScore -ge 70 -and -not $e.EOL -and -not $e.Kev) {

$exploitation = @"
Composite exploitability score indicates elevated real-world attack likelihood.
...
- Listening ports
- Signature trust
- Offensive tooling presence
"@

$remediation = @"
Prioritise based on exposure.

Actions:
- Patch if available
- Reduce service exposure
- Harden configuration
- Monitor exploit telemetry
"@
