...
$exploitation = @"
High exploit density suggests mature public exploit availability.

Attackers may:
- Leverage known RCE exploits
- Chain with privilege escalation
- Deploy automated exploit frameworks
"@
...
"@
    ...
    fncAddFinding `
        -Id ("EXPLOITDENSITY_" + ($Name + $Version -replace '[^A-Za-z0-9]',''))
        -TestId "RECON-INSTALLED-SOFTWARE-INVENTORY-BASIC" `
        -Category "Exploit Surface" `
        -Title "High Public Exploit Density Detected" `
        -Severity "High" `
        -Status "Detected" `
        -Message ("{0} {1} has {2} public exploits indexed (Score={3})." -f $Name,$Version,$exploitCount,$densityScore) `
        -Exploitation $exploitation `
        -Remediation $remediation `
        -Recommendation "Review and patch or remove this software."
        ...
        foreach ($indicator in $toolIndicators) {
            if ($Name -match [regex]::Escape($indicator)) {
                ...
                $exploitation = @"
Offensive or reconnaissance tooling detected.
...
"@
                ...
                fncAddFinding `
                    -Id ("OFFTOOL_BASIC_" + ($Name + $Version -replace '[^A-Za-z0-9]',''))
                    -TestId "RECON-INSTALLED-SOFTWARE-INVENTORY-BASIC" `
                    -Category "Offensive Tooling" `
                    -Title "Offensive / Recon Tool Detected" `
                    -Severity "Medium" `
                    -Status "Detected" `
                    -Message ("{0} {1} identified as offensive tooling." -f $Name,$Version) `
                    -Exploitation $exploitation `
                    -Remediation $remediation `
